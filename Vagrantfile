...
Vagrant.configure(VAGRANTFILE_API_VERSION) do |config|
  ...
  config.vm.provision "shell", inline: "curl --silent -L http://golang.org/dl/go1.3.2.linux-amd64.tar.gz | tar xvz --owner root --group root -C /usr/local && ln -s /usr/local/go/bin/* /usr/local/bin"
